...
def read_dataset(dataset_type):
    ...
    df = pd.read_csv('dataset_{}.csv'.format(dataset_type))  # 读入股票数据
    data = np.array(df['dt'])  # 获取收盘价序列
    ...
    normalize_data = np.c_[normalize_data1, normalize_data2, normalize_data3,normalize_label]
    ...
    for i in range(len(normalize_data) - time_step):#每七天数据预测第八天数据
        ...
        _y = normalize_label[i + time_step]
        X.append(_x.tolist())
        y.append(_y.tolist())
    # plt.figure()
    # plt.plot(data)
